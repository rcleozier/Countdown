...
interface AdContextType {
  adsEnabled: boolean;
  canShowBanner: (screen: string) => boolean;
  canShowInterstitial: (action: string) => Promise<boolean>;
  recordInterstitialShown: () => Promise<void>;
  recordAdShown: (type: 'banner' | 'interstitial', placement: string) => void;
}
...
export const AdProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const canShowBanner = useCallback((screen: string): boolean => {
  ...
  const canShowInterstitial = useCallback(async (action: string): Promise<boolean> => {
    if (!adsEnabled) return false;
  ...
  const recordAdShown = useCallback((type: 'banner' | 'interstitial', placement: string) => {
